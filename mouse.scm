...
(define mouse/xy-set! (foreign-lambda bool "al_set_mouse_xy" display_info integer integer))
...
(define mouse/event-source (foreign-lambda (c-pointer event_source) "al_get_mouse_event_source"))

(bind-opaque-type bitmap (struct ALLEGRO_BITMAP))
(bind-opaque-type cursor (struct ALLEGRO_MOUSE_CURSOR))

(define mouse/make-cursor (foreign-lambda (c-pointer cursor) "al_create_mouse_cursor" (c-pointer bitmap) integer integer))
(define mouse/free-cursor (foreign-lambda void "al_destroy_mouse_cursor" (c-pointer cursor)))
(define mouse/cursor-set! (foreign-lambda bool "al_set_mouse_cursor" display_info (c-pointer cursor)))
(define mouse/show-cursor (foreign-lambda bool "al_show_mouse_cursor" display_info))
(define mouse/hide-cursor (foreign-lambda bool "al_hide_mouse_cursor" display_info))
...
if (al_get_mouse_cursor_position(&x, &y))
   ...
   C_return(C_SCHEME_FALSE);
"))
(define mouse/grab (foreign-lambda bool "al_grab_mouse" display_info))
